...
ax-monitor hook 의 **전송 대상**을 측정 서버 없이 로컬에서 띄워, hook 이 실제로 무엇을
보내는지를 확인하는 **단일 파일 stdlib 서버**(외부 의존 0). raw 기록을 그대로 받아 저장·확인
하고, 필요하면 실서버로 그대로 중계만 한다 — LLM/요약은 없다. 설치 시
`~/.claude/ax-monitor/local-verifier/` 로 복사됩니다.
...
2. 결정적 분해(발화/응답/tool_use/tool_result/skill/agent, tools·turns·tokens)를 stdout
   리포트로 출력 — "무엇이 들어왔나" 확인용(LLM 불필요).
...
python main.py                              # 캡처 + 분해 리포트
python main.py --forward http://<deployed>/v1/sessions   # 로컬 검증 + 실서버로 raw 중계
```
...
| `--forward` / `AX_VERIFY_FORWARD` | (없음) | 실서버로 raw 중계 URL |
