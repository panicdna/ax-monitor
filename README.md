...
서버(Summarizer)로 한 번 보내고, 서버는 결정적 메타 + OpenAI 호환 요약을 추출합니다.
사용자는 평소처럼 `claude` 만 쓰면 됩니다 — 전송은 서버가 수신 즉시 응답하므로 체감 0,
실패/지연해도 세션 종료를 막지 않습니다.
...
헤더를 그대로 저장하고, production 과 동일한 메타 분해 리포트를 출력하며, OpenAI 호환 요약
(미설정 시 stub)과 실서버 중계(`--forward`)를 지원합니다.

```bash
# 로컬 검증 대상 띄우기 (요약 경로까지)
python ~/.claude/ax-monitor/local-verifier/main.py --summarize
...
│       │       ├── main.py · transcript.py · summarize.py
...
- **개인정보 경계** — hook 은 transcript 파일을 그대로 업로드만 합니다. 무엇을 보존/폐기할지는
  서버가 정합니다(원문은 요약 시에만 쓰이고 메타+요약만 적재하는 것이 권장 설계).
